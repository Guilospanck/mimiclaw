...
You send a message on Telegram. The ESP32-S3 picks it up over WiFi, feeds it into an agent loop — the LLM thinks, calls tools, reads memory — and sends the reply back. Supports **Anthropic (Claude)**, **OpenAI (GPT)**, and **OpenRouter** (access free models like Llama, Gemini, Mistral) as providers, switchable at runtime. Everything runs on a single $5 chip with all your data stored locally on flash.
...
- An **Anthropic API key** — from [console.anthropic.com](https://console.anthropic.com), an **OpenAI API key** — from [platform.openai.com](https://platform.openai.com), or an **OpenRouter API key** — from [openrouter.ai](https://openrouter.ai)
...
mimi> set_model_provider openai    # switch provider (anthropic|openai|openrouter)
...
- **Multi-provider** — supports Anthropic (Claude), OpenAI (GPT), and OpenRouter (free models), switchable at runtime
